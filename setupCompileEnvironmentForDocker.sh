...
mkdir /go/src/github.com/docker/docker/keys/ -v
# Add zfs ppa
cp keys/launchpad-ppa-zfs.asc /go/src/github.com/docker/docker/keys/
...
echo 'Acquire::http::Proxy "http://192.168.1.118:8118";' >>/etc/apt/apt.conf
...
git config --global http.proxy "http://192.168.1.118:8118"
git config --global https.proxy "http://192.168.1.118:8118"
export LVM2_VERSION=2.02.103
mkdir -p /usr/local/lvm2 \
	&& curl --proxy http://192.168.1.118:8118 -fsSL "https://mirrors.kernel.org/sourceware/lvm2/LVM2.${LVM2_VERSION}.tgz" \
...
set -x \
	...
	&& curl --proxy http://192.168.1.118:8118 -sSL https://s3.dockerproject.org/darwin/v2/${OSX_SDK}.tar.xz -o "${OSXCROSS_PATH}/tarballs/${OSX_SDK}.tar.xz" \
...
set -x \
	...
	&& curl --proxy http://192.168.1.118:8118 -fsSL "https://github.com/seccomp/libseccomp/releases/download/v${SECCOMP_VERSION}/libseccomp-${SECCOMP_VERSION}.tar.gz" \
...
curl --proxy http://192.168.1.118:8118 -fsSL "https://golang.org/dl/go${GO_VERSION}.linux-amd64.tar.gz" \
	| tar -xzC /usr/local
...
mkdir -p /usr/src/criu \
	&& curl --proxy http://192.168.1.118:8118 -sSL https://github.com/xemul/criu/archive/v${CRIU_VERSION}.tar.gz | tar -v -C /usr/src/criu/ -xz --strip-components=1 \
	&& cd /usr/src/criu \
	&& make \
	&& make install-criu
...
cp contrib/download-frozen-image-v2.sh /go/src/github.com/docker/docker/contrib/
...
cp hack/dockerfile/binaries-commits /tmp/binaries-commits
cp hack/dockerfile/install-binaries.sh /tmp/install-binaries.sh
...
cp . /go/src/github.com/docker/docker
